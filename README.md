...
This repository contains the code necessary to train and test convolutional neural networks (CNNs) for predicting the rotation angle of a text document to correct its orientation. There are scripts to train three models, one on [MNIST](http://yann.lecun.com/exdb/mnist/), another one on the [Google Street View dataset](http://crcv.ucf.edu/data/GMCP_Geolocalization/) and anothe one on the Text Document dataset (not publicly available). Since the data for this application is generated on-the-fly, you can also train using your own images in a similar way. A detailed explanation of the original code and motivation for the project can be found in [this blog](https://d4nst.github.io/).
...
## Train
Run either `python train/train_mnist.py` to train on MNIST or `python train/train_street_view.py` to train on the Google Street View or `python train/train_text_docs.py` to train on Text Document dataset. Note that the first time you run the scripts will take longer since the datasets will be automatically downloaded. Also, you will need a decent GPU to train the *ResNet50* model that is used in `train_street_view.py`, otherwise it will take quite long to finish.
